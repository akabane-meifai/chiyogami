(function (root, factory) {
  ...
  class CustomClient {
    ...
    parseAcceptHeader(acceptHeader) {
      if (!acceptHeader) return [{ type: '*/*', params: {} }];
      return acceptHeader
        .split(',')
        .map((item, index) => {
    ...
    session(url, protocols) {
      const urlObj = new URL(url, typeof location !== 'undefined' ? location.href : undefined);
      const protocol = urlObj.protocol.toLowerCase();
      if (typeof this._handlers[protocol]?.session === 'function') {
        return this._handlers[protocol].session(url, protocols);
      }
      let ws;
      return new TransformStream({
          ...
          return new Promise((resolve, reject) => {
            ...
            ws.onclose = (event) => {
              if (event.wasClean) {
                ...
                controller.error(new Error(`WebSocket closed unexpectedly (code: ${event.code})`));
              }
            };
          });
        },
        transform(chunk, controller) {
          if (ws && ws.readyState === WebSocket.OPEN) {
            ...
            controller.error(new Error('WebSocket is not open for sending data'));
          }
        },
        flush() {
    ...
    subscribe(url, eventSourceInit) {
      const urlObj = new URL(url, typeof location !== 'undefined' ? location.href : undefined);
      const protocol = urlObj.protocol.toLowerCase();
      if (typeof this._handlers[protocol]?.subscribe === 'function') {
        return this._handlers[protocol].subscribe(url, eventSourceInit);
      }
      const target = new EventTarget();
      const es = new EventSource(url, eventSourceInit);
      es.onopen = (event) => target.dispatchEvent(new MessageEvent('open', { data: event }));
      es.onmessage = (event) => target.dispatchEvent(new MessageEvent('message', { data: event.data, lastEventId: event.lastEventId, origin: event.origin }));
      es.onerror = (event) => target.dispatchEvent(new MessageEvent('error', { data: event }));
      target.close = () => {
        es.close();
      };
      return target;
